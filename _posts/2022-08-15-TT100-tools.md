...
(base) shay@MSI:/mnt/c/Users/ShayM$ cd ~
```
- Install Juypter and check kernels
(base) shay@MSI:~$ conda --version 
(base) shay@MSI:~$ conda install jupyter
(base) shay@MSI:~$ jupyter kernelspec list
Available kernels:
...
PS C:\Users\ShayM> wsl
(base) shay@MSI:/mnt/c/Users/ShayM$ cd ~
(base) shay@MSI:~$ pip install bash_kernel
...
(base) shay@MSI:~$ python -m bash_kernel.install
...
> Install JavaScript kernel.  The outputs from these are verbose so everthing is abbreviated to commands.
```bash
...
> Install Java kernel. Install Jupyter Kernel Spec for Java (IJava) on your machine [https://github.com/SpencerPark/IJava#install-pre-built-binary](https://github.com/SpencerPark/IJava#install-pre-built-binary)
