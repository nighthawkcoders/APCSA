...
> Package manager install on MacOS
...
> Key Packages need update on MacOS
- In a Terminal install Git and Python3
```bash
...
$ python3 --version # version of python3 installed
$ brew install java # openjdk install
$ java --version # version of runtime
$ javac --version # version of compiler
```
> Anaconda is Environment Manager on MaOS as well as Windows and Linux. This will help us manage our projects.
- Download for MacOS: https://www.anaconda.com/products/distribution
...
> Key Packages needing update on WSL Ubunut
...
```
> Anaconda is an environment manager on Ubuntu as well as MacOs, Windows, and other Linux. This will help us manage our projects.
- We need to find the link for latest Linux-x86 distribution at the bottom of this page: https://www.anaconda.com/products/distribution.  In a WSL Command / Powershell you will do the following replacing location and command according to data in copied link.
```bash
$ cd ~
...
> At this point the task is to prepare for Jupyter Notebooks and Kernels.  <mark>You must a new WSL Command / Powershell</mark>.  Now the WSL prompt should be <mark>prefixed with (base)</mark>.  If not, you need to go back to Anaconda install.
- Open Command / Powershell to set directory to home (~).  If you are not looking like this you need to back up.
```bash
> PS C:\Users\ShayM> wsl  # Windows prompt
(base) shay@MSI:/mnt/c/Users/ShayM$ cd ~ # WSL prompt
(base) shay@MSI:~$ # WSL home
```

> Install Jupyter and check kernels
```bash
(base) shay@MSI:~$ conda --version 
(base) shay@MSI:~$ conda install jupyter # install jupyter
(base) shay@MSI:~$ jupyter kernelspec list # list installed kernels
Available kernels:
  python3    /home/shay/.local/share/jupyter/kernels/python3
pip install bash_kernel # install bash kernel
python -m bash_kernel.install  # build the kernel
```

> Install Bash kernel.
```bash
> PS C:\Users\ShayM> wsl
(base) shay@MSI:/mnt/c/Users/ShayM$ cd ~ # home directory
(base) shay@MSI:~$ pip install bash_kernel # download bash kernel
...
(base) shay@MSI:~$ python -m bash_kernel.install # install kernel
Installing IPython kernel spec
(base) shay@MSI:~$ jupyter kernelspec list # list kernels
Available kernels:
...
```bash
$ sudo apt install python2  # Python2 is required for process
$ conda install nodejs # node is framework for JavaScript kernel
$ npm -version  # node package manager comes with nodejs
$ npm install -g ijavascript  # get the kernel
$ ijsinstall # install it
...
> Install Java kernel.
```bash
$ wget https://github.com/SpencerPark/IJava/releases/download/v1.3.0/ijava-1.3.0.zip  # download IJava kernel as zip
$ sudo apt install unzip  # unzip to extract IJava kernel
$ unzip ijava-1.3.0.zip # unzip downloaded IJava kernel
$ python install.py --user # install IJava kernel
$ jupyter kernelspec list # list kernels
Available kernels:
  bash          /home/shay/.local/share/jupyter/kernels/bash
